...
console.time(`read csv file(s): [\n${indent(`"${files.join('",\n"')}"`)}\n]\n`);

let data = files.reduce((df, file) => df.concat(
  cudf.DataFrame.readCSV(file, { compression: 'infer' })
), new cudf.DataFrame());

console.timeEnd(`read csv file(s): [\n${indent(`"${files.join('",\n"')}"`)}\n]\n`);

data = data.cast({ BaseDateTime: new cudf.TimestampSecond });
